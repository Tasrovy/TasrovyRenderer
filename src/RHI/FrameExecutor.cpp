...
struct FrameExecutor::Impl {
    std::unique_ptr<IFrameExecutorBackend> backend;
};

FrameExecutor::FrameExecutor()
    : impl_(std::make_unique<Impl>()) {
    impl_->backend = createSelectedFrameExecutorBackend();
    if (!impl_->backend) {
        throw std::runtime_error("No RHI frame executor backend was selected");
    }
}
